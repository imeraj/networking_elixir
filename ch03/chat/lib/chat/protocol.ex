defmodule Chat.Protocol do
  ...
  def encode_message(%Broadcast{} = msg) do
    [
      0x02,
      encode_string(msg.from_username),
      encode_string(msg.contents)
    ]
  end
  ...
  defp decode_broadcast(<<
         username_len::16,
         from_username::size(username_len)-binary,
         contents_len::16,
         contents::size(contents_len)-binary,
         rest::binary
       >>) do
    {:ok, %Broadcast{from_username: from_username, contents: contents}, rest}
  end
